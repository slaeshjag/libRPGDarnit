...
int npcSpawn(void *handle, int x, int y, int l, const char *sprite, const char *logic_func) {
	MAIN *m = handle;
	int npc;
	...
	m->npc.npc[npc].sprite = darnitSpriteLoad(m->darnit, sprite, 0, DARNIT_TILESHEET_FORMAT_RGBA);
	darnitSpriteMove(m->npc.npc[npc].sprite, m->npc.npc[npc].x,  m->npc.npc[npc].y);
...
void npcTeleport(void *handle, unsigned int npc, unsigned int x, unsigned int y, unsigned int l) {
	...
	darnitSpriteMove(m->npc.npc[npc].sprite, m->npc.npc[npc].x,  m->npc.npc[npc].y-16);
	npcSetSort(m, npc, m->npc.npc[npc].l, m->npc.npc[npc].y);
