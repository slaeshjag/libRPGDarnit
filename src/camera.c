...
int cameraCalcXt(void *handle) {
	...
	if (m->map.w * m->map.tile_w <= m->map.w * m->map.tile_w - m->cam.screen_w)
		return (m->map.w * m->map.tile_w) - ((m->cam.screen_w + m->map.tile_w) >> 1);
...
int cameraCalcYt(void *handle) {
	...
	if (m->map.h * m->map.tile_h <= m->map.h * m->map.tile_h - m->cam.screen_h)
		return (m->map.h * m->map.tile_h) - ((m->cam.screen_h + m->map.tile_h) >> 1);
...
int cameraLoop(void *handle) {
	...
	tp = darnitTimeLastFrameTook(m->darnit) * m->cam.speed;
